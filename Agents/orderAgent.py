...
from Tools.menuTools import get_menu_item_by_name, load_menu_from_file
from Tools.orderTools import (
    validate_order, format_order_details, parse_order_input, 
    format_order_confirmation, find_menu_matches
)
from OrderAndMenuModels import *
from SharedMemory import SharedMemory
from prompts.orderAgentPrompt import ORDER_AGENT_PROMPT
import os
import json
import re
...
"""
    ...
    def _load_menu(self):
        """Load menu for the price lookup"""
    ...
    def handle_order_modification(self, customer_input: str) -> str:
        ...
        original_items = json.dumps(self.shared_memory.current_order, sort_key = True)
        original_total = self.shared_memory.order_total
        text = customer_input.strip().lower()

        if any(kw in text for kw in ["cancle my order", "cancle order", "cancle it", "forget my order", "abort order", "nevermind, cancle"]):
            self.shared_memory.clear_order()
            return "Your order has been canclled. Would you like to start a new order?"
...
""" 
        ...
        try:
            chain = modified_prompt | self.llm| StrOutputParser()
            response_text = chain.invoke({
                "customer_input": customer_input,
                "current_order": json.dumps(self.shared_memory.current_order, indent = 2)
            })
        except:
            return "I'd be happy to help modify your order. Could you please tell me what specifically would you like to change?"
    ...
    def get_order_summary(self) -> Dict[str, Any]:
        ...
        if not self.shared_memory.current_order:
            return {
                'status': 'empty',
                'memory':'Your order is currently empty',
                'items': [],
                'totals':{
                    'subtotal':0.0,
                    'tax':0.0,
